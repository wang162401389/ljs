...
class API{
    ...
    public  function api($command, $params = array(), $method = 'GET', $multi = false, $url2=null)
    {
        ...
        //请求接口
        //log::w('url = '.json_encode($url).' params ='.json_encode($params).' method='.$method);
        log::i('url = '.json_encode($url).' params ='.json_encode($params).' method='.$method, 'api');
    ...
    public function checkError($result, $interfaceName="", $ip=""){
        if(!C('debug')){
            log::i('java result = '.encode_json($result), 'api');
        }else{
            log::i('java result = '.encode_json($result), 'api');
        }

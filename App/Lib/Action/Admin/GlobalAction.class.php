...
class GlobalAction extends ACommonAction
{
    ...
    */
		...
		if($_POST){
			...
			foreach($data as $key => $v){
				if(in_array($key,$searchKey)){
					...
					if($key=='is_show' && $v==0)$condition['is_show']=array('eq',0);
    ...
    public function partners(){
    	$this->assign('partners_position', C('FRIEND_LINK'));
		...
		$count  = $partners->where($condition)->count(); // 查询满足要求的总记录数   
		$Page = new Page($count,$page_size); // 实例化分页类传入总记录数和每页显示的记录数   
		$show = $Page->show(); // 分页显示输出
		...
		$list = $partners->field($fields)->where($condition)->order($order)->limit($Page->firstRow.','.$Page->listRows)->select();

		$partnersList = $list;
		$partners_p = C('FRIEND_LINK');
			  
		foreach($partnersList as $key => $v){
			foreach($v as $key_s => $v_s){
				if($key_s == 'link_type') $v_s = $partners_p[$v_s];
				else if($key_s == 'is_show'){
					if($v_s==1) $v_s="是";
					else $v_s="否";
				}
				$partnersList[$key][$key_s] = $v_s;
			}
		} 
		
		$partnersArr['partnersList'] = $partnersList;
		$partnersArr['PageBar'] = $show;



		$this->assign('partners_list', $partnersArr['partnersList']);
		$this->assign('pagebar', $partnersArr['PageBar']);
		$this->assign('position', "合作伙伴");
        $this->display();
    }
    ...
    public function addpartners(){
		...
		if(!empty($_FILES['imgfile']['name'])){
			$this->saveRule = date( "YmdHis", time()).rand(0,1000);
			$this->savePathNew = C('ADMIN_UPLOAD_DIR').'partners/'; 
			$info = $this->CUpload();
			$data['link_img'] = $info[0]['savepath'].$info[0]['savename'];
		}
		if(!isset($_POST['pid'])){//新增
			$newid = M('partners')->add($data);
			if(!$newid>0){
				$this->error('添加失败，请确认填入数据正确');
				exit;
			}
				
			$this->assign('jumpUrl', U('/admin/global/partners'));
			$this->success('合作伙伴添加成功');
		}else{//编辑
		
			$data['id']=intval($_POST['pid']);
			$newid = M('partners')->save($data);
			if(!$newid>0){
				$this->error('编辑失败，请确认填入数据正确');
				exit;
			}
			...
			$this->assign('jumpUrl', U('/admin/global/partners'));
			$this->success('合作伙伴编辑成功');
		}
    }

    public function dodeletepartners(){
    	$data = $_POST;
		...
		$idarray = $data['idarr'];	
		$delnum = M('partners')->where("id in ({$idarray})")->delete(); 
		
		if($delnum){
			$a_data['success'] = $rid;
			$a_data['success_msg'] = "合作伙伴删除成功";
			$a_data['aid'] = $idarray;
		}else{
			$a_data['success'] = 0;
			$a_data['error_msg'] = "合作伙伴删除失败";
		}

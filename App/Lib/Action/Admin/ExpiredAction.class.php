...
class ExpiredAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field = "m.user_name,d.borrow_id as id,b.borrow_name,b.product_type,b.danbao,d.status,d.total,d.borrow_id,b.borrow_uid,d.sort_order,sum(d.capital) as capital,sum(d.interest) as interest,sum(d.substitute_money) as substitute_money,d.deadline,b.borrow_duration,b.repayment_type metricunit";
		...
		$list = $this->index_listFilter($list);
		foreach($list as $k => $v){
	...
	private function index_listFilter($list){
		$row=array();
        $end=time();
		foreach($list as $key=>$v){
            $borrow_time=cal_deadline($v['borrow_id']);
            if($end>$borrow_time){
                $v['breakday'] = getExpiredDays($v['deadline']);
                $v['expired_money'] = getExpiredMoney($v['breakday'],$v['capital'],$v['interest']);
                $v['call_fee'] = getExpiredCallFee($v['breakday'],$v['capital'],$v['interest']);

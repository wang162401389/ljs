...
class ToolAction extends HCommonAction {
    ...
    public function index(){
		if($_POST){
			...
			//$risk_reserve = floatval($_POST['risk_reserve']);//风险互助基金
			...
			$repay_detail['risk_reserve'] = 0;//round($amount*$risk_reserve/100,4);//风险互助基金
		...
		$this->assign("leftlist",array(
			1=>array("turl"=>"/tools/tool2.html","type_name"=>"投资计算器"),
				/**0=>array("turl"=>"/tools/tool.html","type_name"=>"借款计算器")**/
			)
		);
		$this->display();
	}

	...
	if($parm['pagesize']){
      ...
      */
      function get_personal_benefit($uid)
      {
            ...
            ->field('sum(interest) as interest, sum(capital) as capital,sum(interest_fee) as fee, sum(jiaxi_money) as jx')
            ...
            ->find();
            // 待收利息
            $benefit['interest_collection'] = $interest_collection['interest']+
                                              $interest_collection['jx']-
                                              $interest_collection['fee']+ 
                                              $transfer_interest_collection['interest']-
                                              $transfer_interest_collection['fee']; 
            // 待收本金
            $benefit['capital_collection'] = $interest_collection['capital'] + 
                                             $transfer_interest_collection['capital'];
            // $benefit['money_collect'] = $benefit['interest_collection'] + $benefit['capital_collection'];
            // print_r($benefit);die;
            return $benefit;
      }
      ...
      */
      ...
      elseif($type==2) $cardnum = substr($cardnum,0,3).str_repeat("*",5).substr($cardnum,strlen($cardnum)-2);//手机号
      ...
      function Fmoney($money){
            ...
            if($dot[1]){
              return $format_money.".".substr($dot[1], 0, 2);    
            } 
...
function get_admin_name($id=false){
	...
	if(!S($stype)){
      ...
      *****************************/
      ...
      $type = $msgconfig['sms']['type'];// type=0 吉信通短信接口   type=1 漫道短信接口   type=2 亿美短信接口 type=4 新亿美短信
      if($type==0){
      $uid=$msgconfig['sms']['user1']; //分配给你的账号
      $pwd=$msgconfig['sms']['pass1']; //密码
      $mob=$mob; //发送号码用逗号分隔
      if(PATH_SEPARATOR==':'){//如果是Linux系统，则执行linux短息接口
      // $url="http://121.37.40.49:8009/sys_port/gateway/?id=%s&pwd=%s&to=%s&content=%s&time=";
      ...
      // $sendurl="http://121.37.40.49:8009/sys_port/gateway/?";
...
#isU',$v,$ar[$k]);
      ...
      ////////////////////////////////////////////////////////新亿美短信接口 开始///////////////////////////////////////////
            ...
            // curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
            // $data=array('cdkey' => $cdkey, 'password' => $password ,'phone' => $mob,'message'=>$content);
      ...
      {
            $debt_info = M("debt_borrow_info")->where(array("old_borrow_id"=>$borrow_id,"borrow_status"=>array("in","2,4")))->select();
            $tui_list = array();
            $i = 0;
            foreach ($debt_info as $key => $value) {
                  $where["borrow_id"] = $value["id"];
                  $where["type"] = 16;
                  $where["status"] = 2;
                  $list = M("sinalog")->where($where)->select();
                  ...
                  if($count > 0){
                        ...
                        array_push($tui_list,$trade_list);
                        $i ++;
                  }else{
                    M("debt_borrow_info")->where(array("old_borrow_id"=>$borrow_id,"borrow_status"=>2))->save(array("borrow_status"=>3));
                  }
                  
            }
            if($i > 0){
                foreach ($tui_list as $list) {
                  foreach ($list as $l) {
                        sinacancelpretrade($l,$type=2);
                  }
                }   
            }
      ...
      function getVerify_ucenter($uid){
            $pre = C('DB_PREFIX');
            $vo = M("members m")->field("m.id,m.user_leve,m.time_limit,m.user_regtype,m.pin_pass,s.id_status,s.company_status,s.phone_status,s.email_status,s.video_status,s.face_status")->join("{$pre}members_status s ON s.uid=m.id")->where("m.id={$uid}")->find();
            $str = "";
            if($vo['user_regtype'] == 1){
                  ...
                  if($vo['company_status']==3) $str.='<a href="'.__APP__.'/member/verify#fragment-4"><img alt="企业审核通过"   title="企业审核通过" src="'.__ROOT__.'/Style/H/images/icon/id.gif"/></a>&nbsp;';
                  else  $str.='<a href="'.__APP__.'/member/verify#fragment-4"><img alt="企业审核未通过"  title="企业审核未通过" src="'.__ROOT__.'/Style/H/images/icon/id_0.gif"/></a>&nbsp;';
            }

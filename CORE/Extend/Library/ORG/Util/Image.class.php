...
class Image {
    ...
    // 中文验证码
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='msyh.ttf', $verifyName='verify') {
        import('ORG.Util.String');
...
//中文加法
    static function GBCal($length=6, $type='png', $width=180, $height=60, $fontface='mantou.ttf', $verifyName='verify') {
        import('ORG.Util.String');
        $cal=array("加","减","乘","除");
        $num=array("拾","壹","贰","叁","肆","伍","陆","柒","捌","玖");
        $op=rand(0,2);
        $num1=rand(2,9);
        $num2=rand(1,$num1);
        switch($op){
        ...
        $code=$num[$num1].$cal[$op].$num[$num2]."等于";
        $_SESSION[$verifyName] = md5($result);
        //  $code = String::randString($length, 4);
        $width = ($length * 45) > $width ? $length * 45 : $width;
        ...
        for ($i = 0; $i < $length; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = String::msubstr($code, $i, 1);
            imagettftext($im,32, mt_rand(-10, 10), 40 * $i + 20, mt_rand(35, 40), $fontcolor, $fontface, $codex);
        }
        Image::output($im, $type);
    }

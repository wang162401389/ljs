function checkday(){
    ...
    $("#second_duration").empty();
	$(option).appendTo("#borrow_duration");
    $(option).appendTo("#second_duration");
}
...
function is_tidan_change(){
    var is_check = $("#is_change").attr("checked");
    if(is_check){
        $("#_is_change").slideDown();
    }
    else{
        $("#_is_change").slideUp();
        $("#is_change").val('');
...
function check_type(object){
    value=$(object).val();

    if(value==1){
        $(".change_container").slideDown();
    }else{
        $("#is_change").removeAttr("checked");
        $(".change_container").slideUp();
        $("#_is_change").slideUp();
        $("#second_duration").val("");
        $("#seconde_rate").val("");
    }

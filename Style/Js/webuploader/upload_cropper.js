(function( $ ) {
	...
	var Module = {
		base: function(param){
			...
			seajs.use("jquery/webuploader", function() {
					...
					return {
						init: function( selectCb ) {
                            ...
                            uploader.on("uploadSuccess", function( file, ret ) {
                                layer.closeAll();
                                if(ret.result){
                                    $(".js-avatar").attr("src", ret.result)
                                    $("#imgSrc").val(ret.result);
                                }else{
                                    $.wrongMsg("图片上传失败");
                                }
                            });
				...
				// ---------------------------------
				var Croper = (function() {
					var $image = $('.js-imgContainer img');
					var btn = $('.js-uploadBtn');
					var isBase64Supported, callback;
					$image.cropper({
						aspectRatio: 1 / 1,
						preview: ".js-imgSide, .js-imgRound",
						done: function(data) {
							//console.log(data);
						}
					});
					function srcWrap( src, cb ) {
						...
						if ( isBase64Supported ) {
							...
							$.ajax(gConfig.apiurl + "/index.php?m=shop&c=webpreview", {
								method: "POST",
								data: {
									src: src,
									storage: "temp"
	...
	$.getCss(gConfig.jsurl+"/jquery/webuploader/cropper.css");
	define("jquery/upload_cropper", function(require, exports, module) {

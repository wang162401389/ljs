...
function(a) {
    ...
    c.creation = function(a, b) {
        ...
        if(c.elem != null) c.elem[h.elemv] = f,
...
function period_date(param){
	var param = param || {},
		startTime = param.startTime || "#start",
		endTime = param.endTime || "#end",
		minTime = param.minTime || "1900-01-01 00:00:00",
		format = param.format || "YYYY-MM-DD hh:mm",
		istime = param.istime == null ? true:param.istime,
		istoday = param.istoday == null ? false:param.istoday;
	var start = {
		elem: startTime,
		format: format,
...
//开始时间 & 结束时间
setTimeout(function(){
	if($("#start").length) period_date();
    if($("#start2").length) period_date({
        startTime: "#start2",
        endTime: "#end2"
    });
    /*身份证*/
	if($("#startCID").length) period_date({
		startTime: "#startCID",
		endTime: "#endCID",
        format: "YYYY-MM-DD",
        istime: false
